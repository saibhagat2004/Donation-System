...
export default function Campaigns() {
  ...
  const handleChange = (e) => {
    ...
    // Clear errors and success message when user starts editing
    setErrors({
      ...errors,
      [name]: ""
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccessMessage(`Campaign "${form.title}" created successfully! ID: ${res.data.campaign_id}`);
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-xl rounded-2xl">
      ...
      {serverError && (
        <div className="mb-4 text-red-600 text-center font-semibold border border-red-300 bg-red-50 p-3 rounded-lg">
          {serverError}
        </div>
      )}
      
      {successMessage && (
        <div className="mb-4 text-green-600 text-center font-semibold border border-green-300 bg-green-50 p-3 rounded-lg">
          {successMessage}
        </div>
      )}
